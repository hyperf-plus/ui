...
export default {
  ...
  mounted() {
    ...
    //监听事件
    this.$bus.on("treeTableReload", (data) => {
      if (_.isObject(data)){
    ...
    // this.$bus.on("treeTableChecked", (checked) => {
    ...
    this.$nextTick(() => {
      ...
      if (this.$refs.toolbarsView !== undefined){
  ...
  destroyed() {
    ...
    try {
      ...
      //this.$bus.off("treeTableChecked");
      this.$bus.off("treeTableReload");
