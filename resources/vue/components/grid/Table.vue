<template>
  <div class="grid-container">
    <div ref="topView">
      ...
      <el-card shadow="never" :body-style="{ padding: 0 }" class="margin-bottom-sm" v-if="attrs.filter.filters.length > 0">
        <div class="filter-form">
          <el-form :inline="true" :model="filterFormData" v-if="filterFormData">
            <el-form-item v-if="attrs.quickSearch">
              <el-input v-model="quickSearch" :placeholder="attrs.quickSearch.placeholder" :clearable="true" @clear="getData" @keyup.enter.native="getData"></el-input>
            </el-form-item>

            <el-form-item v-for="(item, index) in attrs.filter.filters" :key="index" :label="item.label">
              <ItemDisplay v-model="filterFormData[item.column]" :form-item="item" :form-items="attrs.filters" :form-data="filterFormData" />
            </el-form-item>
            <el-form-item>
    ...
    <el-card shadow="never" :body-style="{ padding: 0 }" v-loading="loading">
      <div class="bottom-border" ref="toolbarsView" v-if="attrs.toolbars.show">
        <div class="grid-top-container">
          <div class="grid-top-container-left">
            <BatchActions :routers="attrs.routers" :key_name="attrs.keyName" :rows="selectionRows" :actions="attrs.batchActions" v-if="attrs.selection" />
            <div class="search-view mr-10" v-if="attrs.quickSearch && attrs.filter.filters.length <= 0">
              <el-input v-model="quickSearch" :placeholder="attrs.quickSearch.placeholder" :clearable="true" @clear="getData" @keyup.enter.native="getData">
                <el-button @click="getData" :loading="loading" slot="append">搜索</el-button>
              </el-input>
            </div>
            <div class="flex-c">
              <component v-for="(component, index) in attrs.toolbars.left" :key="component.componentName + index" :is="component.componentName" :attrs="component" />
            </div>
          </div>
          <div class="grid-top-container-right">
            <component v-for="(component, index) in attrs.toolbars.right" :key="component.componentName + index" :is="component.componentName" :attrs="component" />
      ...
      <div>
        <el-table :ref="attrs.ref || 'table'" :data="tableData" :row-key="attrs.attributes.rowKey" :default-sort="default_sort_get" :height="gridHeight" :max-height="attrs.attributes.maxHeight" :stripe="attrs.attributes.stripe" :border="attrs.attributes.border" :size="attrs.attributes.size" :fit="attrs.attributes.fit" :show-header="attrs.attributes.showHeader" :highlight-current-row="attrs.attributes.highlightCurrentRow" :empty-text="attrs.attributes.emptyText" :tooltip-effect="attrs.attributes.tooltipEffect" :default-expand-all="attrs.attributes.defaultExpandAll" @sort-change="onTableSortChange" @selection-change="onTableselectionChange">
          ...
          <template v-for="column in attrs.columnAttributes">
            <el-table-column :type="column.type" :key="column.prop" :column-key="column.columnKey" :prop="column.prop" :label="column.label" :width="column.width" :sortable="column.sortable" :help="column.help" :align="column.align" :fixed="column.fixed" :header-align="column.headerAlign" :show-overflow-tooltip="column.showOverflowTooltip">
          ...
          <el-table-column v-if="!attrs.attributes.hideActions" :label="attrs.attributes.actionLabel" prop="grid_actions" :fixed="attrs.attributes.actionFixed" :min-width="attrs.attributes.actionWidth" :align="attrs.attributes.actionAlign">
            <template slot="header"></template>
            <template slot-scope="scope">
              <Actions v-if="scope.row.grid_actions && !scope.row.grid_actions.hide" :action_list="scope.row.grid_actions.data" :scope="scope" :key_name="attrs.keyName" />
            </template>
          </el-table-column>
        </el-table>
      </div>
      <div ref="pageView" class="table-page padding-xs" v-if="!attrs.hidePage">
        <el-pagination :layout="attrs.pageLayout" :hide-on-single-page="false" :total="pageData.total" :page-size="pageData.pageSize" :current-page="pageData.currentPage" :page-sizes="attrs.pageSizes" :background="attrs.pageBackground" @size-change="onPageSizeChange" @current-change="onPageCurrentChange" />
      </div>
    </el-card>
    ...
    <DialogForm ref="DialogGridFrom" v-if="attrs.dialogForm" :dialogFormWidth="attrs.dialogFormWidth" :dialogForm="attrs.dialogForm" :dialogTitle="attrs.dialogTitle" />
  </div>
</template>
  ...
  mounted() {
    ...
    if (this.$store.getters.thisPage.grids.page) {
      ...
      if (this.attrs.attributes.dataVuex) {
        this.tableData = this._.cloneDeep(this.$store.getters.thisPage.grids.tableData)
      }
      this.pageData = this._.cloneDeep(this.$store.getters.thisPage.grids.pageData)
      this.quickSearch = this._.cloneDeep(this.$store.getters.thisPage.grids.quickSearch)
      this.filterFormData = this._.cloneDeep(this.$store.getters.thisPage.grids.filterFormData)
  ...
  computed: {
    ...
    gridHeight() {
      if (this.attrs.attributes.height === 'auto') {
        return window.innerHeight - 55 - 20 - window.rootFooterHeight - this.topViewHeight - (this.toolbarsViewHeight > 0 ? this.toolbarsViewHeight + 12 : 0) - this.pageViewHeight - this.bottomComponentViewHeight
      }
